...
public class PermissionByMemory implements PermissionFactory {
	private static IRoleService roleService = (IRoleService) SpringUtil.getBean("roleServiceImpl");
	private static HashMap<Integer, HashMap<Long, List<SysObj>>> sysObjMap = new HashMap<Integer, HashMap<Long, List<SysObj>>>();;// 每个角色对应的菜单
	private static List<SysObj> allSysObj = null;// 全部菜单

	@Override
	public List<SysObj> getPermissionList(Long[] roleIds) {
		if (sysObjMap.get(UserUtil.getTenantId()) == null) {
			initData();
		}
		String[] types = { "1", "2" };
	...
	private static void initData() {
		...
		HashMap<Long, List<SysObj>> tenantMap = new HashMap<Long, List<SysObj>>();
		for (SysRole sysRole : list) {
			Long[] roleIds = { sysRole.getId() };
			tenantMap.put(sysRole.getId(), convertSysObjList(roleService.getMenuListByRoleId(roleIds, null)));
		}
		sysObjMap.put(UserUtil.getTenantId(), tenantMap);
	}
	...
	private static List<SysObj> permissionList(Long[] roleIds, String[] types, boolean hasPermission) {
		...
		for (int i = 0; i < roleIds.length; i++) {
			for (SysObj sysObj : sysObjMap.get(UserUtil.getTenantId()).get(roleIds[i])) {
