...
(function($) {
	...
	$.Autocompleter = function(input, options) {
		...
		function selectCurrent() {
			...
			var openUrl = basePath + "openPersonInfo.shtml?id=" + selected.data.id + "&name=" + encodeURI(encodeURI(selected.data.name))
	...
	$.Autocompleter.defaults = {
		inputClass : "ac_input",
		resultsClass : "search-result-value",
	...
	$.Autocompleter.Select = function(options, input, select, config) {
		...
		function init() {
			...
			list = $("#search-result").mouseover(function(event) {
				if (target(event).nodeName && target(event).nodeName.toUpperCase() == 'LI') {
					if ($(target(event)).hasClass("search-result-title"))
						return;
		...
		function moveSelect(step) {
			if (step > 0) {
				if (listItems.slice(active + 1, active + 2).hasClass("search-result-title")) {
					step++;
				}
			} else {

				if (listItems.slice(active - 1, active).hasClass("search-result-title")) {
					step--;
				}
			}
		...
		function fillList() {
			...
			for (var i = 0; i < max; i++) {
				...
				// term)).addClass(i % 2 == 0 ? "ac_even" :
				// "ac_odd").appendTo(list)[0];
				if (data[i].data.roleName == "患者" && patientCount == 0) {
					patientCount++;
					li = $("<li/>").addClass("search-result-title").html('患者').appendTo(list)[0];
					// $.data(li, "ac_data", "");
				}
				if (data[i].data.roleName == "医生" && doctorCount == 0) {
					doctorCount++;
					li = $("<li/>").addClass("search-result-title").html('医生').appendTo(list)[0];
					// $.data(li, "ac_data", "");
				}
				if (data[i].data.roleName == "护士" && nurseCount == 0) {
					nurseCount++;
					li = $("<li/>").addClass("search-result-title").html('护士').appendTo(list)[0];
					// $.data(li, "ac_data", "");
				}
				var li = $("<li/>").html(options.highlight(formatted, term)).addClass("search-result-value").appendTo(list)[0];
