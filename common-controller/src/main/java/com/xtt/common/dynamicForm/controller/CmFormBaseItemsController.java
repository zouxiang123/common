...
public class CmFormBaseItemsController {
	...
	public Map<String, Object> saveItem(CmFormBaseItemsPO record) {
		...
		if (CommonConstants.SUCCESS.equals(status))
			DynamicFormUtil.refresh(UserUtil.getTenantId(), record.getSysOwner());
	...
	public Map<String, Object> delItem(String itemCode) {
		Map<String, Object> map = new HashMap<String, Object>();
		String status = cmFormBaseItemsService.deleteByItemCode(itemCode);
		if (CommonConstants.SUCCESS.equals(status))
			DynamicFormUtil.refresh(UserUtil.getTenantId(), HttpServletUtil.getProjectName());
		map.put(CommonConstants.STATUS, status);
		return map;
	}
