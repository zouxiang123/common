...
import com.xtt.common.common.constants.CmDictConstants;
import com.xtt.common.common.constants.CommonConstants;
import com.xtt.common.common.util.CmDictUtil;
...
public class CmFormItemsController {
	...
	public ModelAndView view() {
		...
		model.addObject(CmDictConstants.SYS_OWNER, CmDictUtil.getListByType(CmDictConstants.SYS_OWNER, HttpServletUtil.getProjectName()));
		return model;
	}
	...
	public Map<String, Object> delConf(@RequestBody CmFormItemsPO[] records) {
		...
		if (resultMap.isEmpty()) {
			if (records != null && records.length > 0)
				DynamicFormUtil.refresh(UserUtil.getTenantId(), records[0].getSysOwner());
			map.put(CommonConstants.STATUS, CommonConstants.SUCCESS);
		} else {
	...
	public Map<String, Object> saveConf(@RequestBody CmFormItemsVO record) {
		...
		if (resultMap.isEmpty()) {
			if (record.getRecords() != null && record.getRecords().length > 0)
				DynamicFormUtil.refresh(UserUtil.getTenantId(), record.getRecords()[0].getSysOwner());
			map.put(CommonConstants.STATUS, CommonConstants.SUCCESS);
		} else {
			map.putAll(resultMap);
			map.put(CommonConstants.STATUS, CommonConstants.WARNING);
	...
	public Map<String, Object> getDetailItems(CmFormItemsPO record) {
		Map<String, Object> map = new HashMap<String, Object>();
		List<CmFormNodes> list = DynamicFormUtil.getByFromId(UserUtil.getTenantId(), record.getFkFormId());
